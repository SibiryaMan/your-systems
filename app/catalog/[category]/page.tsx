    ...
    <div className="flex flex-col min-h-screen bg-white font-sans">
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-16 pt-20">
          ...
          <div className="flex gap-10 mb-20 border-b border-gray-100 pb-8">
            {SUB_NAV.map((item) => (
              <Link 
                key={item.slug} 
                href={`/catalog/${item.slug}`}
                className={`text-[13px] font-black uppercase tracking-[0.2em] transition-all ${
            ...
            {products?.map((item: any) => (
              <div key={item.id} className="group cursor-pointer">
                <div className="aspect-square bg-[#f9fafb] mb-8 border border-gray-100 group-hover:border-blue-600 transition-all flex items-center justify-center relative overflow-hidden">
                   <div className="absolute top-6 left-6 text-[10px] font-black text-gray-200 uppercase tracking-widest italic opacity-40">YourSystems v2.4</div>
                </div>
                <div className="flex justify-between items-end px-2">
                  <div className="flex flex-col">
                    <span className="text-[12px] font-black text-blue-600 uppercase tracking-widest mb-2">{item.brand}</span>
                    <h3 className="text-[20px] font-black uppercase tracking-tighter leading-none text-[#1a1c23]">{item.name}</h3>
                  </div>
                  <span className="text-[26px] font-black italic text-[#1a1c23] ml-6">{item.price} ₽</span>
                </div>
              </div>
            ))}
          </div>
