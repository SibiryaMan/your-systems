...
export default function Sidebar() {
  return (
    <aside className="w-[320px] bg-[#0f1116] text-white p-10 pt-12 sticky top-20 h-[calc(100vh-80px)] flex flex-col border-r border-white/5">
      ...
      <div className="flex-1 overflow-y-auto space-y-12 pr-4 custom-scrollbar">
        <section>
          <h3 className="text-[10px] font-black text-gray-600 uppercase tracking-[0.2em] mb-8 italic">// ХАРАКТЕРИСТИКИ</h3>
          <div className="space-y-5">
            {CAMERA_FILTERS.map(filter => (
              <label key={filter} className="flex items-center group cursor-pointer">
                ...
                <span className="text-[13px] font-bold uppercase text-gray-500 group-hover:text-white transition-colors tracking-tight">{filter}</span>
              </label>
            ))}
          </div>
        </section>
