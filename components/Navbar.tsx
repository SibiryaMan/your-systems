...
export default function Navbar({ pageTitle }: NavbarProps) {
  ...
  return (
    <nav className="h-20 w-full bg-white border-b border-gray-100 sticky top-0 z-[200]">
      {/* Контейнер для выравнивания по сетке каталога */}
      <div className="flex h-full items-center">
        
        {/* ЛОГОТИП (Фиксированная ширина 320px, как у Sidebar) */}
        <div className="w-80 px-8 flex items-center shrink-0">
        ...
        {/* НАЗВАНИЕ ПОДКАТЕГОРИИ (С тем же отступом px-16, что и main контент) */}
        <div className="flex-1 flex items-center justify-between px-16">
          <div className="flex items-center gap-4">
            {pageTitle && (
              <h1 className="text-2xl font-black uppercase tracking-tight text-black leading-none pt-0.5 animate-in fade-in slide-in-from-left-4 duration-500">
                {pageTitle}
              </h1>
            )}
          </div>
          ...
          <div className="flex items-center gap-10">
            ...
            <Link href="/projects" className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 hover:text-black transition-colors">Проектирование</Link>
            <Link href="/install" className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 hover:text-black transition-colors">Монтаж</Link>
          </div>
        </div>
      </div>
    </nav>
