...
export default function Sidebar({ categories = [], currentCategory }: any) {
  ...
  // ФИНАЛЬНЫЙ ПРОФЕССИОНАЛЬНЫЙ РЕЕСТР (NAG/Torus Standard)
  const groupOrder = [
    'Бренд',
    'Тип управления',
    'Порты 10/100Base-T (RJ45)',
    'Порты 10/100/1000Base-T (RJ45)',
    'Порты 1000Base-X (SFP)',
    'Порты 10GBase-X (SFP+)',
    'Тип PoE',
    'Бюджет PoE (Вт)',
    'Рабочая температура',
            ...
            {sortedGroupNames.map((groupName) => (
              <div key={groupName} className="flex flex-col">
                <button 
                  ...
                  className={`w-full flex items-start justify-between text-[10px] font-black uppercase tracking-[0.2em] py-2 transition-colors text-left ${openGroups[groupName] ? 'text-blue-500' : 'text-gray-500 hover:text-white'}`}
                >
                  <span className="whitespace-normal leading-tight pr-4">{groupName}</span>
                  <span className="text-[9px] opacity-30 shrink-0 mt-0.5">{openGroups[groupName] ? '−' : '+'}</span>
                </button>
                {openGroups[groupName] && (
                  <div className="flex flex-col gap-0.5 mt-1 mb-4 pl-1 animate-in fade-in duration-300">
                    {[...groups[groupName]].sort((a:any, b:any) => (parseInt(a.specs?.sort_order) || 999) - (parseInt(b.specs?.sort_order) || 999)).map((item) => (
                      <div key={item.id} className="flex items-center cursor-pointer group py-0.5" onClick={() => toggleFilter(item.slug)}>
                        <div className={`w-3 h-3 border transition-all flex items-center justify-center mr-3 ${selected.includes(item.slug) ? 'bg-blue-600 border-blue-600 shadow-[0_0_8px_rgba(59,130,246,0.3)]' : 'border-white/10 group-hover:border-white/30'}`}>
                          {selected.includes(item.slug) && <div className="w-1.5 h-1.5 bg-white rounded-full shadow-sm" />}
                        </div>
                        <span className={`text-[11px] font-bold uppercase tracking-tight transition-colors ${selected.includes(item.slug) ? 'text-white' : 'text-gray-400 group-hover:text-gray-200'}`}>{item.name}</span>
                      </div>
                    ))}
                  </div>
                )}
                {!openGroups[groupName] && <div className="border-b border-white/5 w-full" />}
              </div>
            ))}
          </div>
        </div>
